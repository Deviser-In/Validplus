...
async def validate_email(request: EmailRequest, rate_limit: bool = Depends(check_rate_limit)):
    try:
        if request.validation_type == "quick":
            result = quick_verifier.verify_email(request.email)
        else:
            result = detailed_verifier.verify_email(request.email)
